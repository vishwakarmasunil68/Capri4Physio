 ...
 */
public class NdtntpExamFragment extends BaseFragment implements WebServicesCallBack {
     ...
     */
    public static NdtntpExamFragment newInstance(String patientId) {
        NdtntpExamFragment fragment = new NdtntpExamFragment();
...
//        mRecyclerView.setAdapter(mAdapter);
    ...
    private void getNtpRecords() {
        ...
        new WebServiceBaseFragment(nameValuePairArrayList, getActivity(), this, CALL_GET_NTP_EXAMS).execute(ApiConfig.GET_NTP_EXAM);

    }

    private void addFragment() {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        AddNTPExamFragment addNeuroFragment = AddNTPExamFragment.newInstance(patientId, "add", null);
        ft.replace(R.id.fragment_container, addNeuroFragment);
        ft.addToBackStack(null);
        ft.commit();
    }

    void updateNeuroReport(NdtResultPOJO ndtResultPOJO) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        AddNTPExamFragment addNeuroFragment = AddNTPExamFragment.newInstance(patientId, "update", ndtResultPOJO);
        ft.replace(R.id.fragment_container, addNeuroFragment);
        ft.addToBackStack(null);
        ft.commit();
    }

    void ViewNeuroFragment(NdtResultPOJO ndtResultPOJO) {
        FragmentTransaction ft = getFragmentManager().beginTransaction();
        AddNTPExamFragment addNeuroFragment = AddNTPExamFragment.newInstance(patientId, "view", ndtResultPOJO);
        ft.replace(R.id.fragment_container, addNeuroFragment);
        ft.addToBackStack(null);
        ft.commit();
    }
    ...
    public void parseExamResponse(String response) {
        ...
        try {
            Gson gson = new Gson();
            NDTPOJO ndtpojo = gson.fromJson(response, NDTPOJO.class);
            if (ndtpojo.getSuccess().equals("true")) {
                Collections.reverse(ndtpojo.getNdtResultPOJOList());
                UsersAdapterNDT mAdapter = new UsersAdapterNDT(ndtpojo.getNdtResultPOJOList(), getActivity(), NdtntpExamFragment.this);
                mRecyclerView.setAdapter(mAdapter);
            } else {
