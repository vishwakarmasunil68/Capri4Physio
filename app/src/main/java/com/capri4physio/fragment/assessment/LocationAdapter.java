...
//        final ProductBooen p = getProduct(position);
            ...
            textnumber.setText(GetLUserList.patientaray.get(position).getCity().toString());
...
//                    view.setBackgroundResource(R.drawable.btnselector);
                   ...
                   numberrr = GetLUserList.patientaray.get(position).getPatientcode().toString();
                    String patientname="";
                    try {
                        patientname = GetLUserList.patientaray.get(position).getName().toString();
                       String  patientCode1 = GetLUserList.patientaray.get(position).getPatientid().toString();
                        patientCode = (GetLUserList.patientaray.get(position).getCity().toString());
                    }
                    catch (Exception e){
                    ...
                    msglocationintent.putExtra("numberrr", numberrr);
...
//            cancel.setBackgroundResource(R.drawable.btnselector);
            cancel.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    ...
                    String nameedit=GetLUserList.patientaray.get(position).getName().toString();
                    added_by=GetLUserList.patientaray.get(position).getPatientcode().toString();
