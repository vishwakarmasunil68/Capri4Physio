 ...
 */
public class ViewEnquiryList extends AppCompatActivity{
    public static ViewEnquiryList getLUserList;
    TextView results;
    EnquiryAdapter contactAdapter;
    public static ArrayList<String> contactDetails;
    InfoApps Detailapp,Detailapp1;
    public static ListView contactList;
    // URL of object to be parsed
    String JsonURL = "http://www.caprispine.in/users/enquirylist";
...
// Creates the Volley request queue
        ...
        if (AppPreferences.getInstance(getApplicationContext()).getUserType().equals("0")){
            startActivity(new Intent(ViewEnquiryList.this,Viewpatientinlist.class));
            finish();
        }
                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            for (int i = 0;  i <jsonArray.length() ; i++) {
                                ...
                                String booking_date = jsonObject.getString("enquiry_date");
                                String enquiry_p_id = jsonObject.getString("patient_code");
                                String enquiry_staff_name = jsonObject.getString("enquiry_staff_name");
                                String booking_starttime = jsonObject.getString("enquiry_time");
                                String first_name = jsonObject.getString("first_name");
                                String last_name = jsonObject.getString("last_name");
                                String visit_type = jsonObject.getString("enquiry_f_name");
                                String enquiry_subject = jsonObject.getString("enquiry_subject");
                                String enquiry_message = jsonObject.getString("enquiry_message");
                                Detailapp = new InfoApps();
                                Detailapp.setName(enquiry_p_id);
                                Detailapp.setPc(first_name+" "+ last_name );
                                Detailapp.setStaffname(enquiry_staff_name);
                                Detailapp.setSubject(enquiry_subject);
                                Detailapp.setMsg(enquiry_message);
                                Detailapp.setSend_date(booking_date);
                                Detailapp.setEnqtime(booking_starttime);
                                Detailapp.setAppname(visit_type);
                                patientaray.add(Detailapp);
                                contactAdapter = new EnquiryAdapter(ViewEnquiryList.this, R.layout.contactlistadap);
...
//                        Toast.makeText(StmtActivity.this,error.toString(),Toast.LENGTH_LONG).show();
            ...
            protected Map<String,String> getParams(){
                ...
                params.put("enquiry_cl_code",item);
